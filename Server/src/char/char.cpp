...
int32 char_make_new_char( struct char_session_data* sd, char* name_, int32 str, int32 agi, int32 vit, int32 int_, int32 dex, int32 luk, int32 slot, int32 hair_color, int32 hair_style, int16 start_job, int32 sex ){
	...
	// Black Core: Ascension allows six curated first-class paths directly
	// from character creation while preserving the original rAthena job IDs.
	bool blackcore_start_job = start_job == JOB_SWORDMAN || start_job == JOB_MAGE ||
		start_job == JOB_ARCHER || start_job == JOB_ACOLYTE ||
		start_job == JOB_MERCHANT || start_job == JOB_THIEF;
