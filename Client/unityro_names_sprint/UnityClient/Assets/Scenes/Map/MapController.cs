...
public class MapController : MonoBehaviour {
    ...
    private async void Awake() {
        ...
        NetworkClient.HookPacket(ZC.NOTIFY_TIME.HEADER, OnNotifyTime);
        NetworkClient.HookPacket(ZC.ACCEPT_ENTER2.HEADER, OnMapReconnectAccepted);
    ...
    private void OnNotifyTime(ushort cmd, int size, InPacket packet) { }

    private void OnMapReconnectAccepted(ushort cmd, int size, InPacket packet) {
        if (!NetworkClient.IsReconnecting || !(packet is ZC.ACCEPT_ENTER2 accepted)) {
            return;
        }

        var mapInfo = NetworkClient.State.MapLoginInfo ?? new MapLoginInfo();
        mapInfo.PosX = accepted.PosX;
        mapInfo.PosY = accepted.PosY;
        mapInfo.Dir = accepted.Dir;
        NetworkClient.State.MapLoginInfo = mapInfo;

        var entity = Session.CurrentSession?.Entity as Entity;
        if (entity != null && PathFinding != null) {
            entity.StopMoving();
            entity.transform.position = new Vector3(
                accepted.PosX,
                PathFinding.GetCellHeight(accepted.PosX, accepted.PosY),
                accepted.PosY
            );
            entity.SetReady(true);
        }
    ...
    private void OnDestroy() {
        ...
        NetworkClient.UnhookPacket(ZC.NOTIFY_STANDENTRY11.HEADER, OnEntitySpawn);
        NetworkClient.UnhookPacket(ZC.NOTIFY_NEWENTRY11.HEADER, OnEntitySpawn);
        NetworkClient.UnhookPacket(ZC.NOTIFY_MOVEENTRY11.HEADER, OnEntitySpawn);
        NetworkClient.UnhookPacket(ZC.NOTIFY_VANISH.HEADER, OnEntityVanish);
        NetworkClient.UnhookPacket(ZC.NOTIFY_MOVE.HEADER, OnEntityMovement);
        NetworkClient.UnhookPacket(ZC.NPCACK_MAPMOVE.HEADER, OnEntityMoved);
        NetworkClient.UnhookPacket(ZC.HP_INFO.HEADER, OnEntityHpChanged);
        NetworkClient.UnhookPacket(ZC.STOPMOVE.HEADER, OnEntityMovement);
        NetworkClient.UnhookPacket(ZC.NOTIFY_EFFECT2.HEADER, OnEffect);
        NetworkClient.UnhookPacket(ZC.RESURRECTION.HEADER, OnEntityResurrected);
        NetworkClient.UnhookPacket(ZC.SPRITE_CHANGE2.HEADER, OnSpriteChanged);
        NetworkClient.UnhookPacket(ZC.ACTION_FAILURE.HEADER, OnActionFailure);
        NetworkClient.UnhookPacket(ZC.NOTIFY_TIME.HEADER, OnNotifyTime);
        NetworkClient.UnhookPacket(ZC.ACCEPT_ENTER2.HEADER, OnMapReconnectAccepted);
