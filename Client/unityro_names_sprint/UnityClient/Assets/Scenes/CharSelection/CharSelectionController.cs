...
public class CharSelectionController : MonoBehaviour {
    ...
    void Start() {
        ...
        BlackCoreWindowDecorator.Apply(textPanel != null ? textPanel.transform.parent.gameObject : gameObject, "Escolha seu Viajante", "Selecione um personagem ou abra um novo caminho");
    ...
    private async void OnCharacterSelectionAccepted(ushort cmd, int size, InPacket packet) {
        if (packet is HC.NOTIFY_ZONESVR2) {
            ...
            await NetworkClient.ChangeServer(mapIp, currentMapInfo.Port, NetworkServerRole.Map);
    ...
    private void SetTextFields(CharacterData character) {
        ...
        for (int i = 0; i < numChildren; i++) {
            switch (i) {
                case 0: // map
                    fields[i].text = BlackCoreLoreService.ResolveMapName(character.MapName);
                    break;
                case 1: // job
                    var rawJobName = CultureInfo.InvariantCulture.TextInfo.ToTitleCase(
                        JobHelper.GetJobName(character.Job, character.Sex));
                    fields[i].text = BlackCoreLoreService.ResolveJobName(rawJobName, character.Job, character.Sex);
                    break;
                case 2: // lv.
    ...
    private void OnDestroy() {
        ...
        NetworkClient.UnhookPacket(HC.NOTIFY_ZONESVR2.HEADER, OnCharacterSelectionAccepted);
        NetworkClient.UnhookPacket(HC.ACCEPT_MAKECHAR.HEADER, OnMakeCharAccepted);
        NetworkClient.UnhookPacket(ZC.ACCEPT_ENTER2.HEADER, OnMapServerLoginAccepted);
    }
