...
public class CharServerSelectionController : MonoBehaviour {
    ...
    private void Start() {
        ...
        BlackCoreWindowDecorator.Apply(LinearLayout != null ? LinearLayout.transform.parent.gameObject : gameObject, "Selecione o Núcleo", "Escolha o servidor para continuar sua jornada");
    ...
    private async System.Threading.Tasks.Task ConnectToCharServer(AC.ACCEPT_LOGIN3 loginInfo, string charIp) {
        await NetworkClient.ChangeServer(charIp, charServerInfo.Port, NetworkServerRole.Character);
    ...
    private void OnDestroy() {
        NetworkClient?.UnhookPacket(HC.ACCEPT_ENTER.HEADER, OnEnterResponse);
    }
