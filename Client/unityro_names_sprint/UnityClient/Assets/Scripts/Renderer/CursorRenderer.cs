...
public class CursorRenderer : MonoBehaviour {
    ...
    async void Start() {
        ...
        try {
            var spriteDataTask = AddressableAssetCache<SpriteData>.LoadAsync("data/sprite/cursors.asset");
            var atlasTask = AddressableAssetCache<Texture2D>.LoadAsync("data/sprite/cursors.png");
            await System.Threading.Tasks.Task.WhenAll(spriteDataTask, atlasTask);

            var spriteData = spriteDataTask.Result;
            ...
            Debug.LogWarning($"Unable to initialize cursor Addressables: {ex.Message}");
            isReady = false;
        }
    }
