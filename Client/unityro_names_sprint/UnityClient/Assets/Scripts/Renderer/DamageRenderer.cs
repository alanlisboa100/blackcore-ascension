...
public class DamageRenderer : MonoBehaviour {
    ...
    private void Awake() {
        InitialStartingAngle = _startingAngle;
    }
    ...
    public void ResetForPool() {
        ...
        start = 0d;
        CurrentType = 0;
        _startingAngle = InitialStartingAngle;
        x = 0f;
        y = 0f;
        z = 0f;
        scale = 0f;
    ...
    public float Display(float amount, double tick, DamageType? type, Entity entity) {
        ...
        if ((CurrentType & DamageType.CRITICAL) > 0) {
            color = BlackCoreUiTheme.Critical;
            Delay = 1800;
            transform.position += Vector3.up * 0.45f;
        } else if ((CurrentType & DamageType.SP) > 0) {
