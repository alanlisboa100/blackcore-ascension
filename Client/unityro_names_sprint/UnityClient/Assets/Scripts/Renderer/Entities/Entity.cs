...
public class Entity : MonoBehaviour, INetworkEntity {
    ...
    private static GameObject SharedDamagePrefab;
    ...
    private void Awake() {
        SharedDamagePrefab ??= (GameObject) Resources.Load("Prefabs/Damage");
        DamagePrefab = SharedDamagePrefab;
    ...
    /// <summary>
    /// Updates the entity name visibility from the centralized pointer selection system.
    /// Keeping pointer raycasts out of every Entity avoids O(n) physics queries per frame.
    /// </summary>
    internal void SetPointerHover(bool isHovered) {
        if (isHovered || BlackCoreNameService.ShouldKeepNameVisible(this)) {
    ...
    private void UnhookPackets() {
        ...
        NetworkClient.UnhookPacket(ZC.NOTIFY_ACT3.HEADER, OnEntityAction);
        NetworkClient.UnhookPacket(ZC.NOTIFY_ACT.HEADER, OnEntityAction);
        NetworkClient.UnhookPacket(ZC.PAR_CHANGE.HEADER, OnParameterChange);
        NetworkClient.UnhookPacket(ZC.LONGPAR_CHANGE.HEADER, OnParameterChange);
        NetworkClient.UnhookPacket(ZC.LONGPAR_CHANGE2.HEADER, OnParameterChange);
        NetworkClient.UnhookPacket(ZC.COUPLESTATUS.HEADER, OnParameterChange);
        NetworkClient.UnhookPacket(ZC.STATUS.HEADER, OnStatsWindowData);
        NetworkClient.UnhookPacket(ZC.NOTIFY_EXP2.HEADER, OnExpReceived);
        NetworkClient.UnhookPacket(ZC.SKILLINFO_LIST.HEADER, OnSkillsUpdated);
        NetworkClient.UnhookPacket(ZC.SKILLINFO_UPDATE.HEADER, OnSkillsUpdated);
        NetworkClient.UnhookPacket(ZC.ATTACK_RANGE.HEADER, OnAttackRangeReceived);
        NetworkClient.UnhookPacket(ZC.ACK_TOUSESKILL.HEADER, OnUseSkillResult);
        NetworkClient.UnhookPacket(ZC.NOTIFY_SKILL2.HEADER, OnEntityUseSkillToAttack);
        NetworkClient.UnhookPacket(ZC.USESKILL_ACK2.HEADER, OnEntityCastSkill);
        NetworkClient.UnhookPacket(ZC.ATTACK_FAILURE_FOR_DISTANCE.HEADER, OnAttackFailureForDistance);
    }
    ...
    private void SetupCanvas() {
        ...
        if (BlackCoreNameService.ShouldKeepNameVisible(this)) {
            Canvas.ShowEntityName();
        }
    ...
    internal void OnSpriteChange(ZC.SPRITE_CHANGE2.LookType type, short value, short value2) {
        switch (type) {
            case ZC.SPRITE_CHANGE2.LookType.LOOK_BASE:
                Status.jobId = value;
                break;
            case ZC.SPRITE_CHANGE2.LookType.LOOK_HAIR:
                Status.hair = value;
                break;
            case ZC.SPRITE_CHANGE2.LookType.LOOK_WEAPON:
                // Modern Athena servers send weapon and shield together for this look update.
                EquipInfo.Weapon = value;
                EquipInfo.Shield = value2;
                break;
            case ZC.SPRITE_CHANGE2.LookType.LOOK_HEAD_BOTTOM:
                EquipInfo.HeadBottom = value;
                break;
            case ZC.SPRITE_CHANGE2.LookType.LOOK_HEAD_TOP:
                EquipInfo.HeadTop = value;
                break;
            case ZC.SPRITE_CHANGE2.LookType.LOOK_HEAD_MID:
                EquipInfo.HeadMid = value;
                break;
            case ZC.SPRITE_CHANGE2.LookType.LOOK_HAIR_COLOR:
                Status.hair_color = value;
                break;
            case ZC.SPRITE_CHANGE2.LookType.LOOK_CLOTHES_COLOR:
                Status.clothes_color = value;
                break;
            case ZC.SPRITE_CHANGE2.LookType.LOOK_SHIELD:
                EquipInfo.Shield = value;
                break;
            case ZC.SPRITE_CHANGE2.LookType.LOOK_RESET_COSTUMES:
                EquipInfo.HeadTop = 0;
                EquipInfo.HeadMid = 0;
                EquipInfo.HeadBottom = 0;
                break;
            case ZC.SPRITE_CHANGE2.LookType.LOOK_ROBE:
                EquipInfo.Robe = value;
                break;
            case ZC.SPRITE_CHANGE2.LookType.LOOK_SHOES:
            case ZC.SPRITE_CHANGE2.LookType.LOOK_BODY:
            case ZC.SPRITE_CHANGE2.LookType.LOOK_BODY2:
                // No matching visual state exists in this client yet. Ignore safely instead of crashing.
                break;
            default:
                return;
        }
    ...
    public async void PlayAudio(string path) {
        ...
        try {
            var clip = await AddressableAssetCache<AudioClip>.LoadAsync(key);
            if (clip != null && AudioSource != null) {
                AudioSource.clip = clip;
                AudioSource.Play();
            }
        } catch (Exception ex) {
            Debug.LogWarning($"Unable to load entity audio '{path}': {ex.Message}");
        }
    }
    ...
    private void OnEntityAttack(EntityActionRequest pkt, Entity srcEntity, Entity dstEntity) {
        ...
        if (dstEntity) {
            ...
            // Process damage. Visual feedback is client-only and only reacts when
            // the local player is the source/target, so large fights do not shake
            // the camera for every packet on screen.
            if (target) {
                BlackCoreCombatFeedback.ReportHit(
                    srcEntity,
                    dstEntity,
                    pkt.damage,
                    pkt.action == ActionRequestType.ATTACK_CRITICAL
                );
                ...
                switch (pkt.action) {
                    ...
                    case ActionRequestType.ATTACK_CRITICAL:
                        target.Damage(
                            pkt.damage,
                            GameManager.Tick + pkt.sourceSpeed,
                            DamageType.DAMAGE | DamageType.CRITICAL
                        );
                        break;
    ...
    public void Damage(float amount, double tick, DamageType? damageType = null) {
        var damageRenderer = DamageRendererPool.Borrow(DamagePrefab);
        var delay = damageRenderer.Display(amount, tick, damageType, this);
        var generation = damageRenderer.Generation;

        if ((damageType & DamageType.COMBO) > 0) {
            var combos = DamageNumbers.FindAll(it => (it.CurrentType & DamageType.COMBO) > 0);
            foreach (var combo in combos) {
                DamageNumbers.Remove(combo);
                DamageRendererPool.Return(combo);
            }
        }

        DamageNumbers.Add(damageRenderer);
        StartCoroutine(ReturnDamageRendererAfterDelay(damageRenderer, delay, generation));
    }

    private IEnumerator ReturnDamageRendererAfterDelay(DamageRenderer renderer, float delay, int generation) {
        yield return new WaitForSeconds(delay);

        // A renderer can be returned early (for example when a combo number is
        // replaced) and borrowed again before this coroutine wakes up.
        if (renderer != null && renderer.Generation == generation && DamageNumbers.Remove(renderer)) {
            DamageRendererPool.Return(renderer);
        }
    }

    public bool IsWalking => EntityWalk != null && EntityWalk.IsWalking;
