...
public class EntityCanvas : MonoBehaviour {
    ...
    public void SetEntityName(string name) {
        var displayName = BlackCoreNameService.ResolveEntityDisplayName(Entity, name);
        EntityName.text = displayName;
        EntityName.color = BlackCoreNameService.GetNameColor(Entity);
        EntityName.outlineColor = BlackCoreNameService.GetNameOutlineColor(Entity);
        EntityName.outlineWidth = Entity != null && Entity.HasAuthority ? 0.20f : 0.12f;
        EntityName.fontStyle = Entity != null && Entity.HasAuthority ? FontStyles.Bold : FontStyles.Normal;

        Vector2 textSize = EntityName.GetPreferredValues(displayName);
        (EntityName.gameObject.transform as RectTransform).sizeDelta = textSize + new Vector2(10f, 3f);
    }
