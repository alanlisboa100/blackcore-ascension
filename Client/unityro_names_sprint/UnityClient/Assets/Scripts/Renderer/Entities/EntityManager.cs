...
public class EntityManager : MonoBehaviour {
    ...
    public async Task<Entity> SpawnItem(ItemSpawnInfo itemSpawnInfo) {
        ...
        var spriteHandle = Addressables.LoadAssetAsync<SpriteData>($"{itemPath}.asset".SanitizeForAddressables());
        var atlasHandle = Addressables.LoadAssetAsync<Texture2D>($"{itemPath}.png".SanitizeForAddressables());

        SpriteData spriteData;
        Texture2D atlas;
        try {
            spriteData = await spriteHandle.Task;
            atlas = await atlasHandle.Task;
        } catch {
            if (spriteHandle.IsValid()) Addressables.Release(spriteHandle);
            if (atlasHandle.IsValid()) Addressables.Release(atlasHandle);
            throw;
        }

        var itemGO = new GameObject(item.identifiedDisplayName);
        var handleOwner = itemGO.AddComponent<AddressableHandleOwner>();
        handleOwner.Add(spriteHandle);
        handleOwner.Add(atlasHandle);
