...
public class SpriteEntityViewer : GameEntityViewer {
    ...
    private WeaponMotionTrail WeaponTrail;
    ...
    public void Start() {
        ...
        if (ViewerType == ViewerType.WEAPON) {
            WeaponTrail = gameObject.GetOrAddComponent<WeaponMotionTrail>();
        }
    ...
    public override void Init(bool reloadSprites = false) {
        ...
        if (Sprites == null || reloadSprites) {
            ...
            try {
                var spriteData = AddressableAssetCache<SpriteData>.LoadSync(path + ".asset");
                var atlas = AddressableAssetCache<Texture2D>.LoadSync(path + ".png");
                ...
                var palette = AddressableAssetCache<Texture2D>.LoadSync(palettePath + ".png");
                if (palette != null) {
                    MeshRenderer.material.SetTexture("_PaletteTex", palette);
                } else {
                    // selected palette doesn't exist, fallback to original
                    palettePath = path + "_pal";
                    palette = AddressableAssetCache<Texture2D>.LoadSync(palettePath + ".png");
                    MeshRenderer.material.SetTexture("_PaletteTex", palette);
                }
            } catch (Exception e) {
        ...
        BlackCoreSpritePolish.Apply(Entity, MeshRenderer, ViewerType);
    ...
    private void InitShadow() {
        ...
        if (SharedShadowSprite == null) {
            var spriteData = AddressableAssetCache<SpriteData>.LoadSync("data/sprite/shadow.asset");
            var atlas = AddressableAssetCache<Texture2D>.LoadSync("data/sprite/shadow.png");
            SharedShadowSprite = spriteData.GetSprites(atlas)[0];
        }

        var spriteRenderer = ShadowObject.AddComponent<SpriteRenderer>();
        spriteRenderer.sprite = SharedShadowSprite;
