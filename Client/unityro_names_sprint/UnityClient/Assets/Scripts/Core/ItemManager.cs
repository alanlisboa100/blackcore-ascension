...
public class ItemManager : MonoBehaviour {
    ...
    private async void OnItemSpamInGround(ushort cmd, int size, InPacket packet) {
        if (PathFinding == null) {
            PathFinding = FindObjectOfType<PathFinder>();
        }
        if (packet is ZC.ITEM_FALL_ENTRY5) {
            ...
            await EntityManager.SpawnItem(new ItemSpawnInfo() {
                AID = pkt.id,
                mapID = pkt.mapID,
    ...
    private void OnDestroy() {
        ...
        NetworkClient.UnhookPacket(ZC.ITEM_FALL_ENTRY5.HEADER, OnItemSpamInGround);
        NetworkClient.UnhookPacket(ZC.ITEM_ENTRY.HEADER, OnItemSpamInGround);
        NetworkClient.UnhookPacket(ZC.ITEM_PICKUP_ACK.HEADER, OnItemPickup);
        NetworkClient.UnhookPacket(ZC.ITEM_DISAPPEAR.HEADER, OnItemDisappear);
        NetworkClient.UnhookPacket(ZC.INVENTORY_ITEMLIST_EQUIP.HEADER, OnInventoryUpdate);
        NetworkClient.UnhookPacket(ZC.INVENTORY_ITEMLIST_NORMAL.HEADER, OnInventoryUpdate);
        NetworkClient.UnhookPacket(ZC.USE_ITEM_ACK2.HEADER, OnUseItemAnswer);
        NetworkClient.UnhookPacket(ZC.ACK_WEAR_EQUIP_V5.HEADER, OnItemEquipAnswer);
        NetworkClient.UnhookPacket(ZC.ACK_TAKEOFF_EQUIP_V5.HEADER, OnItemTakeOffAnswer);
        NetworkClient.UnhookPacket(ZC.DELETE_ITEM_FROM_BODY.HEADER, OnInventoryRemoveItem);
        NetworkClient.UnhookPacket(ZC.EQUIP_ARROW.HEADER, OnEquipAmmo);
    }
